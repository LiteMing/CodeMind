...
```powershell
# 指定待测 EXE
pwsh -File scripts/test-p1-contract.ps1 -Executable build/bin/CodeMind-1.14.0.exe
...
JSON 报告写入 `build/reports/`。仍需人工执行：A 组、D4/D7、F1/F5、F2 的终端日志可见性、F3 的窗口体感、F4 的 help 文本可见性，以及 G 组视觉交互。

2026-07-12 本机结果：`33 PASS / 0 FAIL / 4 SKIP`（包含 Go、前端、VS Code 回归套件）。
...
- [？] A5 二次冲突：乙确认覆盖后、实际 PUT 前让甲再保存一次。乙若再次收到 412，本地草稿必须保持原样，并可重新选择两个动作。
  没看懂什么意思，是接A4再次保存吗，草稿确实甲乙始终一致的

  说明：不是等 A4 完成后再保存，而是制造“乙已经确认覆盖，但乙的覆盖请求尚未写入时，甲又抢先保存”的竞态。普通操作较难稳定卡中这个时间窗，可用浏览器网络限速辅助复现。前端专项单测已经覆盖“覆盖请求再次收到 412 时不丢本地草稿”，本项可作为人工补充而非发布阻断。
- [√] A6 轮询守卫：冲突态持续至少 10 秒，跨越多个 2 秒轮询周期；乙的本地草稿不能被服务端版本替换。
- [√] A7 切图竞态：让保存请求处于慢响应状态时切到另一张图并编辑；旧请求返回后，新图内容不能被替换，且新图仍会继续保存。
1、发现一个问题，是选择“重新加载服务端”时候镜头会重置，稍微有些麻烦人
2、此外，多人同时快速进行脑图编辑的情况下，按理说除非是编辑影响到同一节点，按理说不应该这么多次地产生客户端和服务端不同步，稍微异步同步一下应该也行？否则多人同时开发老是会让人进行本地、服务端二选一不好。
3、下一个问题，自动快照的缓存和管理是怎样设计的？目前前端能看到“历史快照”侧栏的快照保存显示量很多，不同客户端似乎也有不同的手动快照，而且列表超出了侧栏本身范围，也没有找到删除快照功能
