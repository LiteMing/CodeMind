...
func main() {
	...
	if err != nil {
		fail(fmt.Sprintf("open exe: %v", err))
	}
	defer exeFile.Close()

	resourceSet, err := winres.LoadFromEXE(exeFile)
	if err != nil {
		fail(fmt.Sprintf("load exe resources: %v", err))
	}

	if err := verifyEmbeddedIcon(resourceSet, iconPath); err != nil {
		fail(err.Error())
	}
	if err := verifyEmbeddedManifest(resourceSet); err != nil {
		fail(err.Error())
	}
}

func verifyEmbeddedIcon(resourceSet *winres.ResourceSet, iconPath string) error {
	groupCount := 0
...
func verifyEmbeddedManifest(resourceSet *winres.ResourceSet) error {
	manifestCount := 0
	var manifestData []byte

	resourceSet.WalkType(winres.RT_MANIFEST, func(resourceID winres.Identifier, langID uint16, data []byte) bool {
		manifestCount += 1
		if manifestCount == 1 {
	...
	if manifestCount != 1 {
		return fmt.Errorf("expected exactly 1 manifest, found %d", manifestCount)
	}
	if bytes.Contains(manifestData, []byte("{{")) {
		return fmt.Errorf("embedded manifest still contains template placeholders")
	}
